...
use unimarkup_core::{unimarkup::UnimarkupDocument, unimarkup_block::UnimarkupBlockKind, elements::{HeadingBlock, ParagraphBlock, VerbatimBlock}};

use super::{delta_conversions::to_lsp_line_nr, inline_tokens::SemanticInlineTokenizer, TokenType, OpenTokenType, TokenValue};
	...
	fn tokens(&self, open_types: &mut Vec<OpenTokenType>) -> Vec<SemanticToken> { 
		match self {
			UnimarkupBlockKind::Heading(heading) => {
				heading.tokens(open_types)
			},
			UnimarkupBlockKind::Paragraph(paragraph) => {
				paragraph.tokens(open_types)
			},
			UnimarkupBlockKind::Verbatim(verbatim) => {
				verbatim.tokens(open_types)
			}
		}	
	}
}
